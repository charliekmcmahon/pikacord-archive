...
module.exports={
  ...
  run: async(bot,message,args) => {
    ...
    db.add(`shards_` + user.id, args[1])
    if(args[1] > 1) {
      
      return message.channel.send(`**${args[1]}** Redeems has been sent to **${user.username}**`)
      
    }else{
      
      return message.channel.send(`**${args[1]}** Redeem has been sent to **${user.username}**`)
